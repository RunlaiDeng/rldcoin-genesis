...
def verify(root, expected_pin, record=None):
    ...
    if s["operator_program_sha256"] != digest(__file__):
        ...
        if second_path.exists() or second_path.is_symlink():
            ...
            if third:
                third_time = check_revision(third, digest(__file__), 3, {
...
def finalize_heartbeat(snapshot, submit, allowed, pause=time.sleep, attempts=3):
...
def main():
    ...
    def call(op):
        ...
        if op == "start" and s.get("operations_revision", 0) >= 2:
            startup = START_WITH_RECOVERY if s.get("operations_revision") == 3 else START_WITH_BOUNDED_TIMEOUTS
    ...
    def snapshot():
        ...
        return nodes
    def heartbeat():
        if s.get("operations_revision") != 3:
    ...
    try:
        ...
        call("start")
        if s.get("operations_revision") == 3 and any(n["witness_gate"]["mode"] == "READY_VOTE_AUTHORIZED" for n in snapshot()):
