...
CONTINUING_MODE = "UNTIL_REVOKED"
REVOCATION_FILE = "config/operations-revoked"
GUARD = Path(__file__).with_name("run-with-authorization.py")


def authorization_files(root):
    return [root / "config/permanent-genesis.json",
            root / "config/genesis-manifest.json",
            root / "config/admission-benchmark-report.json",
            root / "artifacts/install-manifest.json",
            root / "artifacts/source/manifest.json",
            root / "config/operations-authorization.json",
            *[root / f"config/operations-authorization-{n}.json" for n in range(2, 6)],
            Path(__file__), GUARD]


def authorization_watch(root, pin):
    """Verify a stable signed snapshot; fail closed on revocation or changes."""
    files = authorization_files(root)
    pinned = [(path, digest(path)) for path in files]
    def allowed():
        try:
            marker = root / REVOCATION_FILE
            return not (marker.exists() or marker.is_symlink()) and all(
                not path.is_symlink() and path.is_file() and digest(path) == expected
                for path, expected in pinned)
        except OSError:
            return False
    if verify(root, pin).get("key_authorization_mode") != CONTINUING_MODE or not allowed():
        raise ValueError("continuing authorization changed during verification")
    return allowed
        ...
        if second_path.exists() or second_path.is_symlink():
            ...
            if third:
                ...
                if third_time < second_time:
                    raise ValueError("operating revisions out of order")
                if fourth:
                    fifth_path = root / "config/operations-authorization-5.json"
                    fifth = read(fifth_path) if fifth_path.exists() or fifth_path.is_symlink() else None
                    fourth_hash = fourth["statement"]["operator_program_sha256"] if fifth else digest(__file__)
                    fourth_time = check_revision(fourth, fourth_hash, 4, {
                    ...
                    if fifth:
                        if GUARD.is_symlink() or not GUARD.is_file():
                            raise ValueError("missing continuing-authorization supervisor")
                        fifth_time = check_revision(fifth, digest(__file__), 5, {
                            "previous_authorization_sha256": hashlib.sha256(canonical(fourth)).hexdigest(),
                            "previous_authorized_program_sha256": fourth_hash,
                            **timeout_scope, **RECOVERY_POLICY,
                            "post_commit_checkpoint_sync": "RETAINED_SEMANTIC_SYNC_V1",
                            "key_authorization_mode": CONTINUING_MODE,
                            "software_key_expires_at": None,
                            "supervisor_program_sha256": digest(GUARD),
                            "revocation_file": REVOCATION_FILE,
                        })
                        if fifth_time < fourth_time:
                            raise ValueError("operating revisions out of order")
                        marker = root / REVOCATION_FILE
                        if marker.exists() or marker.is_symlink():
                            raise ValueError("continuing operations authorization revoked")
                        return {**s, "heartbeat_interval_seconds": 600, "operations_revision": 5,
                                "key_authorization_mode": CONTINUING_MODE, "software_key_expires_at": None,
                                "operations_authorization_sha256": digest(fifth_path)}
...
# Retain the frozen lifecycle's artifact and process identity checks. The legacy
# metadata expiry string explicitly names the mode; it is never a fabricated date.
CONTINUING_LIFECYCLE = r'''
EXPIRY_SUPERVISOR="$RLD_M0_AUTHORIZATION_GUARD"
validate_expiry_supervisor() {
  local file="$1" supervisor_pid parent_pid
  jq -e '.format == "RLD-M0-PROCESS-METADATA-V2" and
    .software_key_exception.expires_at == "UNTIL_REVOKED" and
    (.software_key_exception.supervisor.pid | type == "number" and . > 0 and floor == .) and
    (.software_key_exception.supervisor.script_sha256 | test("^[0-9a-f]{64}$"))' "$file" >/dev/null || return 1
...
def main():
    ...
    env = dict(os.environ, RLD_M0_DIR=str(root), RLD_M0_PORT_BASE=str(a.port_base),
               ...
               RLD_M0_KEY_EXPIRES_AT=s["software_key_expires_at"] or CONTINUING_MODE,
               RLD_M0_AUTHORIZATION_GUARD=str(GUARD),
               NO_PROXY="127.0.0.1,localhost,::1", no_proxy="127.0.0.1,localhost,::1")
    script = root / "artifacts/source/tree/deploy/m0-mainnet.sh"
    continuing = s.get("key_authorization_mode") == CONTINUING_MODE
    deadline = (None if continuing else
                datetime.fromisoformat(s["software_key_expires_at"].replace("Z", "+00:00")).timestamp())
    authorized = authorization_watch(root, a.pin) if continuing else lambda: time.time() < deadline
    running = True
    def stop_signal(*_):
        nonlocal running
        running = False
...
wait_converged
''', "bash", str(script)]
        if continuing and op != "stop":
            if command[1:2] != ["-c"]:
                if op != "verify":
                    raise ValueError("unsupported continuing lifecycle command")
                command = ["bash", "-c", 'source "$1"\nverify_m0', "bash", str(script)]
            command[2] = command[2].replace('source "$1"', 'source "$1"\n' + CONTINUING_LIFECYCLE, 1)
    ...
    def heartbeat():
        ...
        height = finalize_heartbeat(snapshot, submit, lambda: running and authorized(),
    ...
    def publish(state):
        ...
        if continuing:
            payload.update(key_authorization_mode=CONTINUING_MODE,
                           operations_authorization_sha256=s["operations_authorization_sha256"])
    ...
    try:
        call("stop")
        if not authorized():
            publish("AUTHORIZATION_REVOKED" if continuing else "KEY_EXCEPTION_EXPIRED"); return
        call("start")
        ...
        call("stop")
        publish(("AUTHORIZATION_REVOKED" if continuing else "KEY_EXCEPTION_EXPIRED") if not authorized() else "STOPPED")
